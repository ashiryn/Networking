...
namespace FluffyVoid.Networking.Udp
{
    ...
    public class NetworkUdpServer : IDisposable
    {
        ...
        /// <summary>
        ///     Heartbeat system used to track active clients and ensure stale clients can
        ///     be detected
        /// </summary>
        ...
        /// </summary>
        public event EventHandler<ClientDisconnectedEventArgs>?
            ClientDisconnected;
        /// <summary>
        ...
        /// <summary>
        ///     Constructor for the server that binds the server to a desired port and
        ///     allows for a custom heartbeat interval
        /// </summary>
        /// <param name="port">The port the server will bind to and listen on</param>
        /// <param name="pongInterval">
        ///     The amount of time to wait before sending PONG
        ///     messages
        /// </param>
        /// <param name="pingInterval">
        ///     The amount of time to wait before disconnecting clients who haven't
        ///     responded with a PING
        ///     messages
        /// </param>
        public NetworkUdpServer(int   port,
                                float pongInterval,
        ...
        /// <summary>
        ///     Listen loop used to allow the server to listen for network traffic without
        ...
        {
            if (_clientList.TryGetValue(clientId,
        ...
        public async Task SendAsync(string clientName, UdpMessage message)
        {
            ...
            foreach (int id in _clientLookupTable[clientName])
            {
                ...
                {
                    sendTasks.Add(_server.SendAsync(message.Buffer,
                                                    message.Length,
                                                    client.Endpoint));
                }
            }
        ...
        /// <summary>
        ///     Handles properly disconnecting any stale client connections that did not
        ///     respond within the Ping window
        /// </summary>
        ...
        /// <summary>
        ///     Helper function used to remove a client entry from the corresponding lookup
        ///     tables
        /// </summary>
        /// <param name="info">
        ///     The client connection information to use for removal of the
        ///     client
        /// </param>
        ...
        /// </summary>
        /// <param name="clientId">
        ///     The client id that is connecting to determine if the
        ///     client is a dropped client, or a new client
        /// </param>
        ...
        private async Task ProcessConnectionMessage(short      clientId,
                                                    IPEndPoint clientEndpoint,
                                                        ...
                                                        connectInfo)
        {
            ...
            if (_clientList.ContainsKey(clientId))
            {
                ...
                {
                    _clientLookupTable[_clientList[clientId].Name]
                        .Remove(clientId);
                }
            ...
            _clientList[currentId] =
                new ClientInformation(currentId, connectInfo.Name,
                                      clientEndpoint);
            ...
            LogManager
                .Log($"{connectInfo.Name} just connected at {clientEndpoint}",
                     nameof(NetworkUdpServer));
            ...
            ClientConnectionInformation clientAckInfo =
                new ClientConnectionInformation(connectInfo.Name, currentId,
                                                true);
