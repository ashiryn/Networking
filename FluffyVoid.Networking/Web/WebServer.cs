...
namespace FluffyVoid.Networking.Web
{
    ...
    public class WebServer
    {
        ...
        /// <summary>
        ///     Event used to notify listeners that a new web socket client has connected
        ///     to the server
        /// </summary>
        ...
        public async Task ListenAsync()
        {
            ...
            {
                LogManager.LogException("Failed to start server",
                                        nameof(WebServer),
                                        ex: ex);
            }
            catch (Exception ex)
        ...
        public virtual void Update(float dt)
        {
            ...
            if (requestToHandle.IsValid)
            {
                WebMethod method =
                    EnumUtility
                        .GetValueFromDescription<WebMethod>(requestToHandle
                            .HttpMethod);
            ...
            {
                LogManager
                    .LogWarning("Received an invalid request from the web",
                                nameof(WebServer));
            }
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="context">
        ///     Reference to the current http context to retrieve a web
        ///     request from
        /// </param>
        private async Task ProcessMessage(HttpListenerContext context)
        {
            try
            {
                ...
                lock (_requestLock)
                {
                    _requestMessages.Enqueue(new WebServerMessage(context,
                                                 request,
                                                 response));
                }
            }
